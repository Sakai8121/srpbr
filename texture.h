...
vector4_t* load_tex_impl(const char* tex_path, int& width, int& height, bool is_srgb, bool vertical_flip = true)
{
	...
	for (int i = 0; i < height; ++i)
	{
		for (int j = 0; j < width; ++j)
		{
			...
			bool enable_log = (std::string(tex_path).find("./resource/myhdr/prefilter_mip_0") != std::string::npos);
			// --- 間引きログ（例: 64ピクセルごと） ---
			if (enable_log && (i % 64 == 0) && (j % 64 == 0) && (i == j)) {
				printf("[PNG LOG] texel(%4d,%4d) = (%.3f, %.3f, %.3f, %.3f)\n",
					j, i, c.x, c.y, c.z, c.w);
			}
		}
	}
...
vector4_t sample_bilinear(vector4_t* texture, int width, int height, const texcoord_t& texcoord)
{
	...
	vector4_t result = lerp(tu0, tu1, v_weight);
...
/*
	   ...
	   |  +z  |
	   |face2 |
 ｡ｪ｡ｪ｡ｪ｡ｪ｡ｪ｡ｪ ｡ｪ｡ｪ｡ｪ｡ｪ｡ｪ｡ｪ ｡ｪ｡ｪ｡ｪ｡ｪ｡ｪ｡ｪ ｡ｪ｡ｪ｡ｪ｡ｪ｡ｪ｡ｪ
|  -x  |  -y  |  +x  |  +y  |
